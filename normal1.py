...
class NormalGraph(Scene):
    def construct(self):
        ...
        line1 = VGroup()
        for x in np.arange(lower_x, 0, std/2):
            line = Line(
                        start=axes.c2p(x, 0),
        ...
        line2 = VGroup()
        for x in np.arange(0, upper_x, std/2):
            line = Line(
                        start=axes.c2p(x, 0),
                        end=axes.c2p(x+std/2, 0),
                        stroke_color=BLUE,
        ...
        line1.set_opacity(0)
        line2.set_opacity(0)
        self.play(FadeTransformPieces(line1, area1),FadeTransformPieces(line2, area2))

        all_objs.add(area1, area2)
